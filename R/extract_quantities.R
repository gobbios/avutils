...
#' @param turntakingthresh numeric, the threshold for the time between onsets of two subsequent segments to be considered a turn. Default is \code{1} second.
#' @param tiernames a list with two elements. By default set to \code{NULL}, where tier names are guessed from the data type. If supplied, element 1 (\code{targetchild}) should be a character of length one, and the second element (\code{adults}) a character vector of any length. See details.
#'
#' @details if the supplied file is an .rttm file, the following tier names are assumed: \code{list(targetchild = "CHI", adults = c("FEM", "MAL"))}. If supplying an .eaf file, tier names are assumed as follows: \code{list(targetchild = "CHI", adults = c("FA1", "MA1"))}.
#'
...
extract_quantities <- function(path, turntakingthresh = 1, tiernames = NULL) {
  # determine data type
