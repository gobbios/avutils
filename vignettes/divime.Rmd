...
The idea here is to provide R functions that wrap up the shell commands normally used to invoke the virtual machine with the DiViMe tools. The functions are by and large calls using the `system2()` functions (or `system()`). There is now also an early version of a shiny app (try `divime_app()`).
...
Currently, there are the following functions available to run specific DiViMe tools:
  ...
  - `divime_run_all()`: runs all the above tools in one go (except the word count for now)
